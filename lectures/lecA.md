...
**※注意※**  
Minecraftのデフォルトリソースパックの中身は、あくまで参考にするために使用しましょう。デフォルトリソースパックの中の画像をそのまま再配布してしまうのは、(自分の知る限り)許可されている訳ではないと思うのでやめておきましょう。わざわざ何かを言われるなんてことは無いでしょうが、人の画像を勝手に再配布するのは倫理的にアレっぽい気がするので…。~(まあこんなこと言いましたが、あんまりここを人に厳しくしすぎるのも野暮ったいので、「再配布を回避できる場合は必ず回避する」くらいのつもりでもいいかもしれません。)~(あとあくまで素人の個人的意見なので、今言ったことが本当に正しいとは言い切れません。自己責任でお願いします。)
...
Java版のブロック/アイテムのモデル用の場合、"Java Block/Item" を選択します。クリックすると、次はこのような画面になります。

![12](https://user-images.githubusercontent.com/92374693/159626196-f0e6bdac-c4e0-4511-bb35-7cada134bc24.png)

こちらはファイル名、テクスチャサイズなど、プロジェクトのメタデータを設定する画面です。後で設定できるので、飛ばしてしまっても構いません。  

![image](https://user-images.githubusercontent.com/92374693/159626800-547ed28f-368e-441d-8a05-26fda2413315.png)

conformもしくはcancelをクリックすると、モデリング画面に移行します。先ほどのプロジェクトのメタデータは、左上の`File > Project` から再び編集できます。

言語を日本語にしたい場合は、`File > Preferances > Settings` で設定ウィンドウが開き、`General`タブの`Language`の項目で日本語を選択しましょう。

![image](https://github.com/Keeema-1/CustomModelEntity/edit/main/materials/14.gif)

〜使い方〜

出力したファイルを`models/item/stick.json`として保存すると、棒のモデルが変更されます。
