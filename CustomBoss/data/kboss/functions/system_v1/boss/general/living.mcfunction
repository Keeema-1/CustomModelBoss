...
execute if score @s kb.temp = @s kb.temp2 run function kboss:system/boss/general/active
...
# HP用エンティティが足りない&プレイヤーが近い or HP用エンティティが足りている&HPが0以下 -> 死亡
# 死亡(HPが0以下)
execute store result score @s kb.temp if entity @e[tag=KB.BossCheck,tag=KB.AI]
execute if score @s kb.temp = @s kb.temp2 unless score $system kb.health matches 1.. run function kboss:system/boss/each/die
#execute unless score @s kb.temp = @s kb.temp2 if entity @a[distance=..32] run function kboss:system/boss/each/die
