...
execute positioned ~-1 ~2 ~-1 if entity @a[dx=1,dy=0,dz=1,gamemode=!creative,gamemode=!spectator] at @a[dx=1,dy=0,dz=1,gamemode=!creative,gamemode=!spectator,limit=1] run function kboss:system/child/water_bullet/hit

execute at @s if data storage kboss:temp {check:0b} unless block ~ ~3 ~ #kboss:space_and_barrier positioned ~ ~3 ~ run function kboss:system/child/water_bullet/hit
