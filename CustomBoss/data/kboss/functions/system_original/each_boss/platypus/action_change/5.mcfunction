
# 噛みつき

function kboss:system/each_boss/platypus/pose/bite/front/1

scoreboard players set @s kb.action 5
scoreboard players set @s kb.action_sub 1

scoreboard players set @s kb.action_time 40

tag @e[distance=..16,tag=KB.AI,tag=KB.BossCheck] remove KB.Attack
execute as @e[distance=..16,tag=KB.AI,tag=KB.BossCheck] run data merge entity @s {NoAI:1b,Invulnerable:0b}

#tag @e[distance=..16,tag=KB.Body1,tag=KB.AI,tag=KB.BossCheck] add KB.Attack
#execute as @e[distance=..16,tag=KB.Body1,tag=KB.AI,tag=KB.BossCheck] run data merge entity @s {NoAI:0b,Invulnerable:0b}
