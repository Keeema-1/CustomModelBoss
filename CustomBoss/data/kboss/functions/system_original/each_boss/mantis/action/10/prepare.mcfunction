...
execute if data storage kboss:temp {direction:{leftside:1b}} at @s rotated ~ 0 run tp @s ^ ^ ^-0.05 ~-2 ~
execute if data storage kboss:temp {direction:{leftside:0b}} at @s rotated ~ 0 run tp @s ^ ^ ^-0.05 ~2 ~
...
execute if score @s kb.action_time matches ..0 run function kboss:system/each_boss/mantis/action/10/to0
