...
function kboss:system/boss/each/martellos/pose/run/1
...
execute if entity @s[tag=!KB.Angry] run scoreboard players set @s kb.action_time 190
execute if entity @s[tag=KB.Angry] run scoreboard players set @s kb.action_time 185
...
# 攻撃力
execute as @e[distance=..16,tag=KB.HitBox,tag=KB.BossCheck] store result entity @s Attributes[{Name:"minecraft:generic.attack_damage"}].Base double 0.5 run scoreboard players get @e[distance=..16,tag=KB.Main,tag=KB.BossCheck,limit=1] kb.attack_damage
