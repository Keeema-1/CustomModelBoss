...
execute store result score $hitbox_num kb.temp if entity @e[distance=..16,tag=KB.BossCheck,tag=KB.HitBox]
...
# 死亡(HPが0以下)
execute store result score @s kb.temp if entity @e[tag=KB.BossCheck,tag=KB.HitBox]
...
execute unless score $hitbox_num kb.temp = $hitbox_num_correct kb.temp if entity @a[distance=..32] run function kboss:system/boss/general/disappear_timer
execute if score @s kb.disappear_timer matches 1.. run scoreboard players remove @s kb.disappear_timer 1
